...
use serde::Serialize;
#[cfg(not(feature = "disable-db-dump"))]
use std::fs;
use std::path::Path;
use unwrap::unwrap;
...
pub(crate) fn new_db<D: AsRef<Path>, N: AsRef<Path>>(
    ...
    if init_mode == Init::New {
        log::trace!("Creating database at {}", db_path.display());
    ...
    log::trace!("Loading database at {}", db_path.display());
    let result = PickleDb::load_bin(db_path.clone(), PickleDbDumpPolicy::AutoDump);
    if let Err(ref error) = &result {
        log::error!("Failed to load {}: {}", db_path.display(), error);
    }
    Ok(result?)
}

#[cfg(feature = "disable-db-dump")]
pub(crate) fn new_db<D: AsRef<Path>, N: AsRef<Path>>(
    db_dir: D,
    db_name: N,
    _: Init,
    ...
    let db_path = db_dir.as_ref().join(db_name);
    log::trace!("Creating database at {}", db_path.display());
    Ok(PickleDb::new_bin(db_path, PickleDbDumpPolicy::NeverDump))
}
