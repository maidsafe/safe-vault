...
impl Cache {
    ...
    pub fn insert_into_ongoing_gets(&mut self, idle_holder: &XorName, data_idv: &IdAndVersion) {
        let _ = self.ongoing_gets.insert(*idle_holder, (Instant::now(), *data_idv));
    }

    pub fn handle_get_success(&mut self, src: &XorName, data_id: &DataIdentifier, version: &u64) {
    ...
    pub fn handle_get_failure(&mut self, src: &XorName, data_id: &DataIdentifier) -> bool {
    ...
    pub fn contains_or_insert(&mut self, src: &XorName, data_idv: &IdAndVersion) -> bool {
    ...
    pub fn add_records(&mut self, data_idv: IdAndVersion, holders: Vec<XorName>) {
        for holder in holders {
            let _ = self.data_holders.entry(holder).or_insert_with(HashSet::new).insert(data_idv);
        }
    }

    pub fn is_in_unneeded(&self, data_id: &DataIdentifier) -> bool {
        self.unneeded_chunks.iter().any(|id| id == data_id)
    }

    pub fn add_as_unneeded(&mut self, data_id: DataIdentifier) {
        self.unneeded_chunks.push_back(data_id);
    }

    pub fn get_records_in_cache(&self) -> Vec<IdAndVersion> {
    ...
    pub fn prune_unneeded_chunks<T: ContactInfo>(&mut self,
    ...
    pub fn clean_unneeded_chunks(&mut self) -> Option<DataIdentifier> {
        self.unneeded_chunks.pop_front()
    }

    /// Remove entries from `ongoing_gets` that are no longer responsible for the data or that
    /// disconnected.
    pub fn prune_ongoing_gets<T: ContactInfo>(&mut self, routing_table: &RoutingTable<T>) -> bool {
    ...
    pub fn needed_data(&mut self) -> Vec<(XorName, IdAndVersion)> {
...
impl DataManager {
    ...
    pub fn handle_refresh(&mut self,
        ...
        for data_idv in data_list {
            if !self.cache.contains_or_insert(src, &data_idv) {
    ...
    fn clean_chunk_store(&mut self) {
        while self.chunk_store_full() {
            if let Some(data_id) = self.cache.clean_unneeded_chunks() {
                let _ = self.chunk_store.delete(&data_id);
            } else {
                break;
            }
