...
use routing::{self, GROUP_SIZE, Data, RoutingTable, XorName};
...
pub fn verify_kademlia_invariant_for_all_nodes(nodes: &[TestNode]) {
    let routing_tables : Vec<RoutingTable> =
            nodes.iter().map(|node| node.routing_table().clone()).collect();
    for node_index in 0..routing_tables.len() {
