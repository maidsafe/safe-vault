...
#[cfg(test)]
 mod test {
    ...
    fn get_put_content<'a>(action: &RoutingNodeAction) -> GenericSendableType {
        match *action {
            RoutingNodeAction::Put{destination: _, content: ref content} => content.clone(),
            _ => unimplemented!()
        }
    }
    ...
    fn churn_test() {
        ...
        {// MaidManager - churn handling
            ...
            assert!(get_put_content(&churn_data[0]).name() == from);
            // MaidManagerAccount
            let sendable: GenericSendableType = get_put_content(&churn_data[0]);
        ...
        {// DataManager - churn handling
            ...
            assert!(get_put_content(&churn_data[0]).name() == data.name().clone());
        ...
        {// VersionHandler - churn handling
            ...
            let churn_data = vault.handle_churn(Vec::<NameType>::with_capacity(0));
            assert_eq!(churn_data.len(), 1);
            assert_eq!(get_put_content(&churn_data[0]).name(), data.name());
